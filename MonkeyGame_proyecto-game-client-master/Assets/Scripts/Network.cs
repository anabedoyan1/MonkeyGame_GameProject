...
public class Network : MonoBehaviour
{
    ...
    public void SendCharacter(TeamEnum _team, RoleEnum _role, string playerId, string nickname)
    {        
        PlayerData data = new PlayerData();

        if(_team == TeamEnum.Blue)
            data.team = "Blue";        
        if (_team == TeamEnum.Red)        
            data.team = "Red";
        
        if (_role == RoleEnum.Monkey)
            data.role = "Monkey";
        if (_role == RoleEnum.CocoShooter)
            data.role = "Coco shooter";
        
        data.playerId = playerId;
        socket.Emit("playerCharacterization", JsonUtility.ToJson(data));
    }
}
